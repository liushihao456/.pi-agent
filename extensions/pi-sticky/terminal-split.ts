...
export class TerminalSplitCompositor {
	...
	private getScrollableRows(): number {
		const shouldUseRawRows =
			this.disposed ||
			this.writing ||
			this.renderingCluster ||
			this.checkingOverlay ||
			this.hasVisibleOverlay();
		if (shouldUseRawRows) return this.getRawRows();
	...
	private refreshRootWindow(width: number): number {
		...
		if (
			nextScrollOffset !== this.scrollOffset ||
			this.maxScrollOffset !== previousMaxScrollOffset
		) {
			this.pendingImageCleanup = true;
		}
	...
	private getSelectedText(): string {
		const hasCompleteSelection =
			this.selectionArea && this.selectionAnchor && this.selectionFocus;
