...
// ---------------------------------------------------------------------------
// Working message / indicator using pi's official API (setWorkingMessage,
// setWorkingIndicator, setWorkingVisible) — no Loader.prototype patching.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Spinner frames — used via setWorkingIndicator instead of Loader patches
// ---------------------------------------------------------------------------

function getDefaultSpinnerFrames(): string[] {
	const chars = process.env.TERM === "xterm-ghostty" || process.platform === "darwin"
		? ["·", "✢", "✳", "✶", "✻", "✽"]
		: ["·", "✢", "*", "✶", "✻", "✽"];
	return [...chars, ...[...chars].reverse()];
}

const SPINNER_FRAMES = getDefaultSpinnerFrames();
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		// Use pi's official API for the spinner animation — set once, persists for the session
		try {
			ctx.ui.setWorkingIndicator({ frames: SPINNER_FRAMES, intervalMs: 250 });
		} catch { /* noop */ }
	});
